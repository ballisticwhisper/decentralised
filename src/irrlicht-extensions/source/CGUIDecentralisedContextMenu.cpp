...
namespace irr
{
	namespace gui
	{
		...
		//! constructor
		CGUIDecentralisedContextMenu::CGUIDecentralisedContextMenu(IGUIEnvironment* environment,
		...
		//! destructor
		CGUIDecentralisedContextMenu::~CGUIDecentralisedContextMenu()
		{
		...
		void CGUIDecentralisedContextMenu::setCloseHandling(ECONTEXT_MENU_CLOSE onClose)
		{
			CloseHandling = onClose;
		}

		//! get current behavior when the menue will be closed
		ECONTEXT_MENU_CLOSE CGUIDecentralisedContextMenu::getCloseHandling() const
		{
			return CloseHandling;
		}

		//! Returns amount of menu items
		u32 CGUIDecentralisedContextMenu::getItemCount() const
		{
			return Items.size();
		}
		...
		//! Adds a menu item.
		u32 CGUIDecentralisedContextMenu::addItem(const wchar_t* text, s32 commandId, bool enabled, bool hasSubMenu, bool checked, bool autoChecking)
		{
			return insertItem(Items.size(), text, commandId, enabled, hasSubMenu, checked, autoChecking);
		}

		//! Insert a menu item at specified position.
		u32 CGUIDecentralisedContextMenu::insertItem(u32 idx, const wchar_t* text, s32 commandId, bool enabled,
		...
		s32 CGUIDecentralisedContextMenu::findItemWithCommandId(s32 commandId, u32 idxStartSearch) const
		{
		...
		void CGUIDecentralisedContextMenu::setSubMenu(u32 index, CGUIDecentralisedContextMenu* menu)
		{
			if (index >= Items.size())
				return;
		...
		void CGUIDecentralisedContextMenu::addSeparator()
		{
		...
		const wchar_t* CGUIDecentralisedContextMenu::getItemText(u32 idx) const
		{
			if (idx >= Items.size())
				return 0;
		...
		void CGUIDecentralisedContextMenu::setItemText(u32 idx, const wchar_t* text)
		{
			if (idx >= Items.size())
				return;
		...
		void CGUIDecentralisedContextMenu::setItemAutoChecking(u32 idx, bool autoChecking)
		{
			if (idx >= Items.size())
				return;
		...
		bool CGUIDecentralisedContextMenu::getItemAutoChecking(u32 idx) const
		{
			if (idx >= Items.size())
				return false;
		...
		bool CGUIDecentralisedContextMenu::isItemEnabled(u32 idx) const
		{
			if (idx >= Items.size())
			{
		...
		bool CGUIDecentralisedContextMenu::isItemChecked(u32 idx) const
		{
			if (idx >= Items.size())
			{
				_IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
		...
		void CGUIDecentralisedContextMenu::setItemEnabled(u32 idx, bool enabled)
		{
			if (idx >= Items.size())
				return;
		...
		void CGUIDecentralisedContextMenu::setItemChecked(u32 idx, bool checked)
		{
			if (idx >= Items.size())
				return;
		...
		//! Removes a menu item
		void CGUIDecentralisedContextMenu::removeItem(u32 idx)
		{
			if (idx >= Items.size())
				return;
		...
		void CGUIDecentralisedContextMenu::removeAllItems()
		{
			for (u32 i = 0; i<Items.size(); ++i)
				if (Items[i].SubMenu)
					Items[i].SubMenu->drop();
		...
		bool CGUIDecentralisedContextMenu::OnEvent(const SEvent& event)
		{
			if (isEnabled())
			{
		...
		void CGUIDecentralisedContextMenu::setVisible(bool visible)
		{
			HighLighted = -1;
		...
		u32 CGUIDecentralisedContextMenu::sendClick(const core::position2d<s32>& p)
		{
			u32 t = 0;
		...
		bool CGUIDecentralisedContextMenu::highlight(const core::position2d<s32>& p, bool canOpenSubMenu)
		{
			if (!isEnabled())
			{
				return false;
		...
		core::rect<s32> CGUIDecentralisedContextMenu::getHRect(const SItem& i, const core::rect<s32>& absolute) const
		{
		...
		core::rect<s32> CGUIDecentralisedContextMenu::getRect(const SItem& i, const core::rect<s32>& absolute) const
		{
			core::rect<s32> r = absolute;
			r.UpperLeftCorner.Y += i.PosY;
			r.LowerRightCorner.Y = r.UpperLeftCorner.Y + i.Dim.Height;
		...
		void CGUIDecentralisedContextMenu::draw()
		{
			if (!IsVisible)
				return;
			...
			// draw frame
			skin->draw2DRectangle(this, skin->getColorFromSkin(L"MenuBar_MenuBackgroundColor"), AbsoluteRect, clip);
			...
			for (s32 i = 0; i<(s32)Items.size(); ++i)
			{
				if (Items[i].IsSeparator)
				{
					// draw separator
					rect = AbsoluteRect;
					rect.UpperLeftCorner.Y += Items[i].PosY + 5;
					rect.LowerRightCorner.Y = rect.UpperLeftCorner.Y + 1;
					rect.UpperLeftCorner.X += 5;
					rect.LowerRightCorner.X -= 5;

					skin->draw2DRectangle(this, skin->getColorFromSkin(L"MenuBar_SeperatorColor"), rect, clip);
				...
				else
				{
					...
					if (i == HighLighted && Items[i].Enabled)
					{
						...
						skin->draw2DRectangle(this, skin->getColorFromSkin(L"MenuBar_MenuHighlightColor"), r, clip);
					}
		...
		void CGUIDecentralisedContextMenu::recalculateSize()
		{
		...
		s32 CGUIDecentralisedContextMenu::getSelectedItem() const
		{
			return HighLighted;
		}
		...
		IGUIContextMenu* CGUIDecentralisedContextMenu::getSubMenu(u32 idx) const
		{
			if (idx >= Items.size())
				return 0;
		...
		s32 CGUIDecentralisedContextMenu::getItemCommandId(u32 idx) const
		{
			if (idx >= Items.size())
				return -1;
		...
		void CGUIDecentralisedContextMenu::setItemCommandId(u32 idx, s32 id)
		{
			if (idx >= Items.size())
				return;
		...
		void CGUIDecentralisedContextMenu::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options = 0) const
		{
		...
		void CGUIDecentralisedContextMenu::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options = 0)
		{
			...
			if (Parent && (Parent->getType() == EGUIET_CONTEXT_MENU || Parent->getType() == EGUIET_MENU))
				((CGUIDecentralisedContextMenu*)Parent)->setSubMenu(in->getAttributeAsInt("ParentItem"), this);
		...
		void CGUIDecentralisedContextMenu::setEventParent(IGUIElement *parent)
		{
			EventParent = parent;
		...
		bool CGUIDecentralisedContextMenu::hasOpenSubMenu() const
		{
			for (u32 i = 0; i<Items.size(); ++i)
		...
		void CGUIDecentralisedContextMenu::closeAllSubMenus()
		{
			for (u32 i = 0; i<Items.size(); ++i)
				if (Items[i].SubMenu)
