...
namespace irr
{
	namespace gui
	{
		...
		//! constructor
		CGUIDecentralisedDropdown::CGUIDecentralisedDropdown(IGUIEnvironment* environment, IGUIElement* parent,
			...
			Selected(-1), HAlign(EGUIA_UPPERLEFT), VAlign(EGUIA_CENTER), MaxSelectionRows(5), HasFocus(false)
		{
			...
			ButtonNormal = skin->getTextureFromSkin(L"Dropdown_ButtonNormal");

			s32 width = 15;
			if (ButtonNormal)
				width = ButtonNormal->getSize().Width;

			core::rect<s32> r;
			r.UpperLeftCorner.X = rectangle.getWidth() - width;
			r.LowerRightCorner.X = rectangle.getWidth();

			r.UpperLeftCorner.Y = 0;
			r.LowerRightCorner.Y = rectangle.getHeight();
		...
		void CGUIDecentralisedDropdown::setText(std::wstring text)
		{
			SelectedText->setText(text.c_str());
		}

		void CGUIDecentralisedDropdown::setTextAlignment(EGUI_ALIGNMENT horizontal, EGUI_ALIGNMENT vertical)
		{
			HAlign = horizontal;
			VAlign = vertical;
		...
		void CGUIDecentralisedDropdown::setMaxSelectionRows(u32 max)
		{
		...
		u32 CGUIDecentralisedDropdown::getMaxSelectionRows() const
		{
		...
		u32 CGUIDecentralisedDropdown::getItemCount() const
		{
			return Items.size();
		}
		...
		const wchar_t* CGUIDecentralisedDropdown::getItem(u32 idx) const
		{
			if (idx >= Items.size())
				return 0;
		...
		//! returns string of an item. the idx may be a value from 0 to itemCount-1
		u32 CGUIDecentralisedDropdown::getItemData(u32 idx) const
		{
			if (idx >= Items.size())
				return 0;
		...
		s32 CGUIDecentralisedDropdown::getIndexForItemData(u32 data) const
		{
		...
		void CGUIDecentralisedDropdown::removeItem(u32 idx)
		{
			if (idx >= Items.size())
				return;
		...
		const wchar_t* CGUIDecentralisedDropdown::getText() const
		{
		...
		u32 CGUIDecentralisedDropdown::addItem(const wchar_t* text, u32 data)
		{
		...
		void CGUIDecentralisedDropdown::clear()
		{
			Items.clear();
			setSelected(-1);
		}
		...
		s32 CGUIDecentralisedDropdown::getSelected() const
		{
			return Selected;
		}
		...
		void CGUIDecentralisedDropdown::setSelected(s32 idx)
		{
		...
		bool CGUIDecentralisedDropdown::OnEvent(const SEvent& event)
		{
			if (isEnabled())
			{
		...
		void CGUIDecentralisedDropdown::sendSelectionChangedEvent()
		{
			if (Parent)
			{
				SEvent event;
		...
		void CGUIDecentralisedDropdown::draw()
		{
			if (!IsVisible)
				return;
			...
			SelectedText->setBackgroundColor(skin->getColorFromSkin(L"Textbox_BackgroundColor"));
			if (isEnabled())
			{
			...
			// background
			rect.LowerRightCorner.X = AbsoluteRect.LowerRightCorner.X - width;
			driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_BackgroundColor"), rect, &AbsoluteClippingRect);
		...
		void CGUIDecentralisedDropdown::openCloseMenu()
		{
			if (ListBox)
			{
				// close list box
		...
		void CGUIDecentralisedDropdown::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options = 0) const
		{
		...
		void CGUIDecentralisedDropdown::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options = 0)
		{
