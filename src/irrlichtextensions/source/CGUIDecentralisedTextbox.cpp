...
namespace irr
{
	namespace gui
	{
		...
		//! constructor
		CGUIDecentralisedTextbox::CGUIDecentralisedTextbox(const wchar_t* text, bool border,
		...
		//! destructor
		CGUIDecentralisedTextbox::~CGUIDecentralisedTextbox()
		{
			if (OverrideFont)
				OverrideFont->drop();
		...
		void CGUIDecentralisedTextbox::setOverrideFont(IGUIFont* font)
		{
		...
		IGUIFont * CGUIDecentralisedTextbox::getOverrideFont() const
		{
			return OverrideFont;
		}

		//! Get the font which is used right now for drawing
		IGUIFont* CGUIDecentralisedTextbox::getActiveFont() const
		{
			if (OverrideFont)
				return OverrideFont;
		...
		void CGUIDecentralisedTextbox::setOverrideColor(video::SColor color)
		{
			OverrideColor = color;
		...
		video::SColor CGUIDecentralisedTextbox::getOverrideColor() const
		{
		...
		void CGUIDecentralisedTextbox::setDrawBorder(bool border)
		{
			Border = border;
		}

		//! Sets whether to draw the background
		void CGUIDecentralisedTextbox::setDrawBackground(bool draw)
		{
			Background = draw;
		}

		//! Sets if the text should use the overide color or the color in the gui skin.
		void CGUIDecentralisedTextbox::enableOverrideColor(bool enable)
		{
			OverrideColorEnabled = enable;
		}

		bool CGUIDecentralisedTextbox::isOverrideColorEnabled() const
		{
		...
		void CGUIDecentralisedTextbox::setWordWrap(bool enable)
		{
			WordWrap = enable;
			breakText();
		}
		...
		void CGUIDecentralisedTextbox::updateAbsolutePosition()
		{
		...
		bool CGUIDecentralisedTextbox::isWordWrapEnabled() const
		{
			_IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
			return WordWrap;
		}
		...
		void CGUIDecentralisedTextbox::setMultiLine(bool enable)
		{
			MultiLine = enable;
			breakText();
		}
		...
		bool CGUIDecentralisedTextbox::isMultiLineEnabled() const
		{
			_IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
			return MultiLine;
		}
		...
		void CGUIDecentralisedTextbox::setPasswordBox(bool passwordBox, wchar_t passwordChar)
		{
		...
		bool CGUIDecentralisedTextbox::isPasswordBox() const
		{
			_IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
			return PasswordBox;
		}
		...
		void CGUIDecentralisedTextbox::setTextAlignment(EGUI_ALIGNMENT horizontal, EGUI_ALIGNMENT vertical)
		{
			HAlign = horizontal;
			VAlign = vertical;
		}
		...
		bool CGUIDecentralisedTextbox::OnEvent(const SEvent& event)
		{
			if (isEnabled())
			{
		...
		bool CGUIDecentralisedTextbox::processKey(const SEvent& event)
		{
		...
		void CGUIDecentralisedTextbox::draw()
		{
			if (!IsVisible)
				return;
			...
			{
				skin->draw2DRectangle(this, skin->getColorFromSkin(L"Textbox_BackgroundColor"), AbsoluteRect, &AbsoluteClippingRect);
			}
			...
			if (Border)
			{
				...
				// background
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_BackgroundColor"), AbsoluteRect, &AbsoluteClippingRect);

				// inner shadow
				rect = AbsoluteRect;
				rect.LowerRightCorner.Y = rect.UpperLeftCorner.Y + 2;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_InnerShadowColor"), rect, &AbsoluteClippingRect);

				++rect.UpperLeftCorner.Y;
				rect.LowerRightCorner.Y = AbsoluteRect.LowerRightCorner.Y;
				rect.LowerRightCorner.X = rect.UpperLeftCorner.X + 2;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_InnerShadowColor"), rect, &AbsoluteClippingRect);	// left

				rect = AbsoluteRect;
				++rect.UpperLeftCorner.Y;
				rect.UpperLeftCorner.X = rect.LowerRightCorner.X - 2;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_InnerShadowColor"), rect, &AbsoluteClippingRect);	// right

				rect = AbsoluteRect;
				++rect.UpperLeftCorner.X;
				rect.UpperLeftCorner.Y = AbsoluteRect.LowerRightCorner.Y - 3;
				--rect.LowerRightCorner.X;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_InnerShadowColor"), rect, &AbsoluteClippingRect);	// bottom

				// top and side borders
				rect = AbsoluteRect;
				rect.LowerRightCorner.Y = rect.UpperLeftCorner.Y + 1;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_TopBorderColor"), rect, &AbsoluteClippingRect);

				++rect.UpperLeftCorner.Y;
				rect.LowerRightCorner.Y = AbsoluteRect.LowerRightCorner.Y;
				rect.LowerRightCorner.X = rect.UpperLeftCorner.X + 1;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_TopBorderColor"), rect, &AbsoluteClippingRect);	// left

				rect = AbsoluteRect;
				++rect.UpperLeftCorner.Y;
				rect.UpperLeftCorner.X = rect.LowerRightCorner.X - 1;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_TopBorderColor"), rect, &AbsoluteClippingRect);	// right

				rect = AbsoluteRect;
				++rect.UpperLeftCorner.X;
				rect.UpperLeftCorner.Y = AbsoluteRect.LowerRightCorner.Y - 2;
				--rect.LowerRightCorner.X;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_BottomBorderColor"), rect, &AbsoluteClippingRect);	// bottom

				// lower shadow
				rect = AbsoluteRect;
				rect.UpperLeftCorner.Y = AbsoluteRect.LowerRightCorner.Y - 1;
				driver->draw2DRectangle(skin->getColorFromSkin(L"Textbox_LowerShadowColor"), rect, &AbsoluteClippingRect);	// bottom

				//skin->draw3DSunkenPane(this, skin->getColorFromSkin(L"Textbox_BackgroundColor"), false, Background, AbsoluteRect, &AbsoluteClippingRect);
		...
		void CGUIDecentralisedTextbox::setText(const wchar_t* text)
		{
			Text = text;
		...
		void CGUIDecentralisedTextbox::setAutoScroll(bool enable)
		{
			AutoScroll = enable;
		}
		...
		bool CGUIDecentralisedTextbox::isAutoScrollEnabled() const
		{
			_IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
			return AutoScroll;
		}
		...
		core::dimension2du CGUIDecentralisedTextbox::getTextDimension()
		{
			core::rect<s32> ret;
		...
		//! infinity.
		void CGUIDecentralisedTextbox::setMax(u32 max)
		{
			Max = max;
		...
		u32 CGUIDecentralisedTextbox::getMax() const
		{
			return Max;
		}


		bool CGUIDecentralisedTextbox::processMouse(const SEvent& event)
		{
		...
		s32 CGUIDecentralisedTextbox::getCursorPos(s32 x, s32 y)
		{
			IGUIFont* font = getActiveFont();
		...
		void CGUIDecentralisedTextbox::breakText()
		{
			if ((!WordWrap && !MultiLine))
				return;
		...
		void CGUIDecentralisedTextbox::setTextRect(s32 line)
		{
			if (line < 0)
				return;
		...
		s32 CGUIDecentralisedTextbox::getLineFromPos(s32 pos)
		{
			if (!WordWrap && !MultiLine)
				return 0;
		...
		void CGUIDecentralisedTextbox::inputChar(wchar_t c)
		{
			if (!isEnabled())
				return;
		...
		void CGUIDecentralisedTextbox::calculateScrollPos()
		{
			if (!AutoScroll)
				return;
		...
		void CGUIDecentralisedTextbox::calculateFrameRect()
		{
		...
		//! set text markers
		void CGUIDecentralisedTextbox::setTextMarkers(s32 begin, s32 end)
		{
		...
		void CGUIDecentralisedTextbox::sendGuiEvent(EGUI_EVENT_TYPE type)
		{
			if (Parent)
			{
				SEvent e;
		...
		void CGUIDecentralisedTextbox::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options = 0) const
		{
		...
		void CGUIDecentralisedTextbox::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options = 0)
		{
